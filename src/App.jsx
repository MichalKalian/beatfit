...
export default function App(){
  ...
  async function loadWs(wsId){
    try{
      ...
      setWs({id:wR.data.id,name:wR.data.name,code:wR.data.code});
      const uMap={},uIds=[];
  ...
  function selectU(id){
    if(pendU===id){setPendU(null);setPinIn("");setPinErr(false);setSetPin(false);return;}
    setPendU(id);setPinIn("");setPinErr(false);
    const u=users[id];setSetPin(!u?.pin);setNewPin("");setNewPinC("");
  }
  ...
  function loginAs(id){
    ...
    setPendU(null);setPinIn("");setSetPin(false);
  ...
  function logout(){
    ...
    setPendU(null);setWsIn("");setPinIn("");setSetPin(false);setInvInfo(null);
      ...
      {!ws?(
        <>
          <div style={{textAlign:"center",padding:"2rem 0 1.5rem"}}>
            <div className="bf-ws-badge">Firemní soutěž</div>
          ...
          <div className="bf-card">
            <div className="bf-label" style={{marginBottom:10}}>Kód skupiny</div>
            <div style={{display:"flex",gap:8,marginBottom:6}}>
              <input placeholder="Např. BEATFIT" value={wsIn} onChange={e=>{setWsIn(e.target.value.toUpperCase());setWsErr("");}} onKeyDown={e=>e.key==="Enter"&&enterWs()} className={`bf-inp bf-inp-mono${wsErr?" bf-inp-err":""}`} style={{flex:1,fontSize:18,letterSpacing:"0.15em",textAlign:"center"}}/>
            ...
            <p style={{margin:"10px 0 0",fontSize:12,color:"var(--bf-text3)",fontFamily:"var(--bf-font)"}}>Kód dostaneš od osoby, která skupinu vytvořila.</p>
          </div>
          <div style={{...P,borderTop:"1.5px solid var(--bf-border)",marginTop:"2rem",paddingTop:"1.25rem",paddingLeft:0,paddingRight:0}}>
        ...
        <>
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"1rem 0 0.75rem",borderBottom:"1.5px solid var(--bf-border)",marginBottom:"1.25rem"}}>
            <button onClick={()=>{setWs(null);setPendU(null);}} style={{fontSize:22,background:"none",border:"none",cursor:"pointer",color:"var(--bf-text3)",padding:0,lineHeight:1,fontFamily:"var(--bf-font)"}}>‹</button>
            <div style={{flex:1}}>
            ...
            <div style={{marginBottom:"1.5rem"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {Object.entries(users).sort(([,a],[,b])=>a.name.localeCompare(b.name)).map(([id,u])=>{
                  ...
                  return(
                    <div key={id}>
                      <button onClick={()=>selectU(id)} className={`bf-user-row${sel?" selected":""}`} style={{gap:12}}>
                      ...
                      {sel&&(
                        <div className="bf-pin-panel">
                          {setPin?(
                            <>
                              <p style={{margin:"0 0 10px",fontSize:13,color:"var(--bf-text2)",fontFamily:"var(--bf-font)",fontWeight:500}}>Nastav si PIN pro přihlašování (min. 4 číslice)</p>
                            ...
                            <>
                              <input type="password" inputMode="numeric" maxLength={6} placeholder="PIN" value={pinIn}
                                onChange={e=>{setPinIn(e.target.value.replace(/\D/g,""));setPinErr(false);}}
                                onKeyDown={e=>e.key==="Enter"&&verifyPin()} autoFocus
                                className={`bf-inp-pin${pinErr?" err":""}`} style={{marginBottom:pinErr?6:10}}/>
          ...
          <div className="bf-card">
            ...
            <input type="password" inputMode="numeric" maxLength={6} placeholder="PIN (min. 4 číslice)" value={newRegPin} onChange={e=>setNewRegPin(e.target.value.replace(/\D/g,""))} className="bf-inp-pin" style={{marginBottom:12}}/>
  ...
  if(view==="admin")return(
    <div style={P}>
      <div className="bf-topbar">
        <div>
          <div className="bf-label">Panel</div>
          <p style={{margin:0,fontWeight:800,fontSize:17,fontFamily:"var(--bf-font)",color:"var(--bf-text)"}}>Administrátor</p>
        </div>
      ...
      {adminTab==="workspaces"&&<>
        <div className="bf-section-header"><div className="bf-label" style={{margin:0}}>Skupiny</div></div>
        {editWs?(
          <div className="bf-card" style={{marginBottom:"1rem"}}>
            ...
            <div style={{display:"flex",gap:8}}><button onClick={updateWs} className="bf-btn" style={{flex:1}}>Uložit</button><button onClick={()=>setEditWs(null)} className="bf-btn-ghost" style={{flex:1}}>Zrušit</button></div>
          </div>
        ):(
          <>
              ...
              {Object.entries(allWs).map(([id,w])=>{const uc=(allWsU[id]||[]).length;return(
                <div key={id} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px",background:"var(--bf-surface)",border:"1.5px solid var(--bf-border-md)",borderRadius:"var(--bf-r-md)"}}>
                  <div style={{flex:1}}>
                    ...
                    <p style={{margin:0,fontSize:11,color:"var(--bf-text3)",fontFamily:"var(--bf-mono)",letterSpacing:"0.08em"}}>{w.code} · {uc} {uc===1?"hráč":"hráčů"}</p>
                  </div>
                  ...
                  <button onClick={()=>deleteWs(id)} className="bf-btn-danger" style={{fontSize:12,padding:"6px 12px"}}>Smazat</button>
                </div>
              );})}
              {!Object.keys(allWs).length&&<p style={{fontSize:13,color:"var(--bf-text3)",fontFamily:"var(--bf-font)"}}>Zatím žádné skupiny.</p>}
            </div>
            <div className="bf-card">
              ...
              <div style={{display:"flex",gap:8,marginBottom:12,alignItems:"center"}}>
                <input placeholder="Kód" value={nWsCode} onChange={e=>setNWsCode(e.target.value.toUpperCase())} className="bf-inp bf-inp-mono" style={{flex:1,fontSize:18,letterSpacing:"0.12em"}}/>
                <button onClick={()=>setNWsCode(randCode())} className="bf-btn-ghost" style={{flexShrink:0,fontSize:12}}>↻</button>
              </div>
              <button onClick={createWs} disabled={!nWsName.trim()||!nWsCode.trim()} className="bf-btn">Vytvořit skupinu →</button>
            </div>
          </>
        )}
      </>}
      ...
      {adminTab==="players"&&<>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:"1rem"}}>
          {Object.entries(allWs).map(([id,w])=>(
            <button key={id} onClick={()=>setASelWs(id)} className={`bf-chip${aSelWs===id?" active":""}`} style={{flex:"none",padding:"7px 16px"}}>{w.name}</button>
          ))}
        </div>
        {aSelWs&&<>
          {editUser?(
            <div className="bf-card">
              ...
              <div style={{display:"flex",gap:8}}><button onClick={saveEditUser} className="bf-btn" style={{flex:1}}>Uložit</button><button onClick={()=>setEditUser(null)} className="bf-btn-ghost" style={{flex:1}}>Zrušit</button></div>
            </div>
          ):(
              ...
              {(allWsU[aSelWs]||[]).sort((a,b)=>a.name.localeCompare(b.name)).map(u=>(
                <div key={u.id} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",background:"var(--bf-surface)",border:"1.5px solid var(--bf-border-md)",borderRadius:"var(--bf-r-md)"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <p style={{margin:0,fontSize:13,fontWeight:700,color:"var(--bf-text)",fontFamily:"var(--bf-font)"}}>{u.name}</p>
                    <p style={{margin:0,fontSize:10,color:"var(--bf-text3)",fontFamily:"var(--bf-font)"}}>{calcAge(u.dob)} let · {u.entryCount} záz. · PIN: {u.pin?<span style={{color:"var(--bf-success)"}}>✓</span>:<span style={{color:"var(--bf-danger)"}}>❗</span>}</p>
                  </div>
            ...
            return(
              <div key={a.key} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 14px",background:"var(--bf-surface)",border:`1.5px solid ${changed?"var(--bf-warn)":"var(--bf-border-md)"}`,borderRadius:"var(--bf-r-md)"}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <p style={{margin:0,fontSize:10,color:"var(--bf-text3)",fontFamily:"var(--bf-font)"}}>výchozí: {DEFAULT_PTS[a.key]} b/{a.unit}</p>
                </div>
                <input type="number" min="0" step="0.01" value={current} onChange={e=>setPtsEdit(p=>({...p,[a.key]:parseFloat(e.target.value)||0}))} className="bf-act-num" style={{width:74}}/>
    ...
    return(
      <div style={P}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:"1rem"}}>
          {[
            {label:"Skóre",val:score.toFixed(1),color:undefined},
            {label:"Věk. koef.",val:`×${ageMult(age).toFixed(2)}`,color:undefined},
            {label:"Streak",val:`${streak}${streak>0?" 🔥":""}`,color:streak>=7?"#f97316":undefined},
          ].map(({label,val,color})=>(
            <div key={label} className="bf-stat">
              <div className="bf-label" style={{marginBottom:4}}>{label}</div>
              <p style={{margin:0,fontSize:20,fontWeight:700,fontFamily:"var(--bf-mono)",color:color||"var(--bf-text)"}}>{val}</p>
            </div>
          ))}
        </div>
        {weekGoal>0&&(
          <div className="bf-surface" style={{marginBottom:"1rem"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <div className="bf-label" style={{margin:0}}>Týdenní cíl</div>
              <span style={{fontSize:12,fontFamily:"var(--bf-mono)",color:"var(--bf-text2)"}}>{weekScore.toFixed(0)} / {weekGoal} b</span>
            </div>
            <div className="bf-progress-bar"><div className="bf-progress-fill" style={{width:`${goalPct}%`,background:goalPct>=100?"var(--bf-success)":"var(--bf-accent)"}}/></div>
            {goalPct>=100&&<p style={{margin:"8px 0 0",fontSize:12,color:"var(--bf-success)",fontWeight:700,fontFamily:"var(--bf-font)"}}>Cíl splněn! 🎉</p>}
          </div>
        )}
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {ACTS.map(a=>(
            <div key={a.key} className="bf-act-row">
              ...
              <div style={{flex:1,minWidth:0}}>
                <p style={{margin:0,fontSize:13,fontWeight:700,fontFamily:"var(--bf-font)",color:"var(--bf-text)"}}>{a.label}</p>
                <p style={{margin:0,fontSize:10,color:"var(--bf-text3)",fontFamily:"var(--bf-font)"}}>{a.pts} b/{a.unit} · {a.sub}</p>
              </div>
          ...
          <div style={{marginBottom:"1.25rem"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {globalSeasons.sort(([,a],[,b])=>b.start_date.localeCompare(a.start_date)).map(([sid,s])=>{
                ...
                return(<button key={sid} onClick={()=>setLbMode(`season:${sid}`)} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",background:"var(--bf-surface)",border:"1.5px solid var(--bf-border-md)",borderRadius:"var(--bf-r-md)",cursor:"pointer",width:"100%",textAlign:"left",transition:"border-color 0.15s"}}>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}><span className={`bf-badge ${lbl.cls}`}>{lbl.text}</span>{st==="active"&&dl>=0&&<span style={{fontSize:11,color:"var(--bf-text3)",fontFamily:"var(--bf-font)"}}>zbývá {dl} dní</span>}</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
          {AM.filter(a=>actW[a.key]).sort((a,b)=>actW[b.key].val-actW[a.key].val).map(a=>(
            <div key={a.key} className="bf-card" style={{padding:"10px 14px"}}>
              ...
              <p style={{margin:0,fontSize:12,color:"var(--bf-text2)",fontFamily:"var(--bf-mono)",fontWeight:500}}>{a.unit==="km"?actW[a.key].val.toFixed(1):Math.round(actW[a.key].val)} {a.unit}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  if(view==="teams"){
    if(teamView==="list")return(
      <div style={P}>
        ...
        <div className="bf-section-header" style={{marginBottom:"1rem"}}>
          <div className="bf-label" style={{margin:0}}>Moje týmy</div>
          <button onClick={()=>setTeamView("create")} className="bf-btn-sm">+ Nový tým</button>
        </div>
          ...
          {myTeamIds.map(tid=>{const t=teams[tid];if(!t)return null;const mc=(members[tid]||[]).length,tsc=Object.values(seasons).filter(s=>s.team_id===tid).length;return(
            <button key={tid} onClick={()=>{setActiveTeam(tid);setTeamView("detail");setTTab("score");setTSeason("all");setShowTSF(false);}} className="bf-team-card">
              ...
              <div style={{flex:1}}>
                ...
                <p style={{margin:0,fontSize:12,color:"var(--bf-text3)",fontFamily:"var(--bf-font)"}}>{mc} {mc===1?"člen":"členů"}{tsc>0?` · ${tsc} výzev`:""}{t.created_by===uid?" · tvůj":""}</p>
              </div>
              <span style={{fontSize:18,color:"var(--bf-text3)"}}>›</span>
            </button>
          );})}
    ...
    if(teamView==="create")return(
      <div style={P}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:"1.5rem"}}>
          ...
          <div className="bf-label" style={{margin:0}}>Nový tým</div>
        </div>
        <div className="bf-card">
          <input placeholder="Název týmu" value={newTName} onChange={e=>setNewTName(e.target.value)} className="bf-inp" style={{marginBottom:12}} onKeyDown={e=>e.key==="Enter"&&createTeam()}/>
          <button onClick={createTeam} disabled={!newTName.trim()||saving} className="bf-btn">{saving?"Vytvářím…":"Vytvořit tým →"}</button>
        </div>
      </div>
    );
    ...
    if(teamView==="detail"&&activeTeam){
      ...
      const lb=(()=>{const t2=todayStr(),w2=weekAgoStr(),res={};
        for(const id of tMids){const u=users[id];if(!u)continue;const days=entries[id]||{};let sc2=0;const acts={};for(const a of AM)acts[a.key]=0;
          ...
          sc2+=calcScore(e,calcAge(u.dob),pts);for(const a of AM)acts[a.key]+=parseFloat(e[a.key])||0;}
          res[id]={sc:sc2,name:u.name,age:calcAge(u.dob),acts};}return res;})();
      const sorted=Object.entries(lb).sort((a,b)=>b[1].sc-a[1].sc);
      const myRank=sorted.findIndex(([id])=>id===uid)+1;
      ...
      return(
        <div style={P}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:"1rem"}}>
            <button onClick={()=>setTeamView("list")} style={{fontSize:22,background:"none",border:"none",cursor:"pointer",color:"var(--bf-text3)",padding:0,lineHeight:1}}>‹</button>
            <div style={{flex:1}}><p style={{margin:0,fontSize:16,fontWeight:800,fontFamily:"var(--bf-font)",color:"var(--bf-text)"}}>{team.name}</p><p style={{margin:0,fontSize:12,color:"var(--bf-text3)",fontFamily:"var(--bf-font)"}}>{tMids.length} členů</p></div>
            <button onClick={()=>leaveTeam(activeTeam)} className="bf-btn-danger" style={{padding:"6px 12px",fontSize:12}}>Opustit</button>
          </div>
          <div className="bf-card" style={{marginBottom:"1rem"}}>
            <div className="bf-label" style={{marginBottom:8}}>Pozvánka do týmu</div>
            ...
            <p style={{margin:"8px 0 0",fontSize:11,color:"var(--bf-text3)",fontFamily:"var(--bf-font)"}}>Odkaz obsahuje kód skupiny — příjemce se dostane přímo dovnitř.</p>
          </div>
          <div style={{marginBottom:"1rem"}}>
            ...
            {showTSF&&<SForm onSubmit={()=>createSeason(activeTeam)} onCancel={()=>setShowTSF(false)}/>}
          ...
          <div className="bf-nav" style={{marginBottom:"1rem"}}>
            {[["score","Body"],["activity","Aktivity"]].map(([t,l])=>(<button key={t} onClick={()=>setTTab(t)} className={`bf-nav-btn${tTab===t?" active":""}`}>{l}</button>))}
          </div>
          ...
          {tTab==="activity"&&<><div className="bf-label" style={{marginBottom:8}}>Porovnání aktivit</div>
            {AM.map(a=>{const vals=sorted.map(([id,d])=>({name:d.name,val:d.acts[a.key]||0,isMe:id===uid})).sort((x,y)=>y.val-x.val);const mx=Math.max(...vals.map(v=>v.val),1);if(vals.every(v=>v.val===0))return null;return(
              <div key={a.key} className="bf-card" style={{marginBottom:"0.75rem"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {vals.filter(v=>v.val>0).map((v,i)=>(<div key={v.name}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                      ...
                      <span style={{fontSize:12,fontFamily:"var(--bf-mono)",color:"var(--bf-text)",fontWeight:600}}>{a.unit==="km"?v.val.toFixed(1):Math.round(v.val)} {a.unit}</span>
                    </div>
    ...
    return(
      <div style={P}>
        ...
        <div className="bf-card" style={{marginBottom:"1rem"}}>
          ...
          {weekGoal>0&&<>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:12,color:"var(--bf-text2)",fontFamily:"var(--bf-font)"}}>Tento týden</span><span style={{fontSize:12,fontFamily:"var(--bf-mono)",color:"var(--bf-text2)",fontWeight:600}}>{weekScore.toFixed(0)} / {weekGoal} b</span></div>
            <div className="bf-progress-bar"><div className="bf-progress-fill" style={{width:`${goalPct}%`,background:goalPct>=100?"var(--bf-success)":"var(--bf-accent)"}}/></div>
            {goalPct>=100&&<p style={{margin:"8px 0 0",fontSize:12,color:"var(--bf-success)",fontWeight:700,fontFamily:"var(--bf-font)"}}>Cíl splněn! 🎉</p>}
          </>}
        </div>
        ...
        <div className="bf-card" style={{marginBottom:"1rem",padding:"1rem"}}>
          <div style={{display:"flex",alignItems:"flex-end",gap:3,height:68}}>
            {cDays.map((d,i)=>{const s2=cScores[i],h=s2>0?Math.max(5,Math.round((s2/maxSc)*58)):3;return(
              <div key={d} style={{flex:1,display:"flex",alignItems:"flex-end",height:68}}>
                <div title={`${d}: ${s2.toFixed(1)} b`} className="bf-bar" style={{width:"100%",height:h,background:d===todayStr()?"var(--bf-accent)":s2>0?"#c084fc":"var(--bf-surface3)"}}/>
              </div>
            );})}
          </div>
          ...
          {AM.filter(a=>totals[a.key]>0).map(a=>(
            <div key={a.key} className="bf-card" style={{padding:"10px 14px"}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}>
                <div className="bf-act-icon" style={{width:24,height:24,borderRadius:6,fontSize:10,background:a.color+"20",color:a.color}}>{a.icon}</div>
                <span style={{fontSize:11,color:"var(--bf-text3)",fontFamily:"var(--bf-font)"}}>{a.label}</span>
              </div>
              <p style={{margin:0,fontSize:17,fontWeight:700,fontFamily:"var(--bf-mono)",color:"var(--bf-text)"}}>{a.unit==="km"?totals[a.key].toFixed(1):Math.round(totals[a.key])}<span style={{fontSize:11,fontWeight:500,color:"var(--bf-text3)",marginLeft:3}}>{a.unit}</span></p>
            </div>
          ))}
        </div>
        <div className="bf-section-header" style={{marginBottom:8}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          ...
          {dates.map(d=>(
            <div key={d} className="bf-card" style={{padding:"10px 14px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <span style={{fontSize:12,fontWeight:600,fontFamily:"var(--bf-mono)",color:"var(--bf-text2)"}}>{d}</span>
                <span style={{fontSize:14,fontWeight:700,fontFamily:"var(--bf-mono)",color:"var(--bf-text)"}}>{calcScore(myDays[d],age,pts).toFixed(1)} b</span>
              </div>
