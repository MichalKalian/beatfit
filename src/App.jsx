...
function exportCSV(userName, days, pts, age) {
  ...
  const rows = Object.entries(days).sort(([a],[b])=>a.localeCompare(b)).map(([date, e]) => {
    return [date, calcScore(e, age, pts).toFixed(2), ...ACTIVITY_META.map(a => parseFloat(e[a.key]) || 0)];
  });
...
export default function App() {
  ...
  const [teams, setTeams]         = useState({});       // id -> { name, created_by, invite_code }
  const [members, setMembers]     = useState({});       // team_id -> [user_id, ...]
  ...
  const [teamView, setTeamView]   = useState("list");   // list | create | detail
  const [activeTeam, setActiveTeam] = useState(null);
  const [newTeamName, setNewTeamName] = useState("");
  const [teamPeriod, setTeamPeriod] = useState("week");
  const [teamTab, setTeamTab]     = useState("score");  // score | activity
  const [pendingInvite, setPendingInvite] = useState(null); // invite code from URL
  const [inviteInfo, setInviteInfo] = useState(null);   // { teamId, teamName } for banner
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [{ data: uD, error: ue }, { data: eD, error: ee },
             { data: sD }, { data: gD },
             { data: tD, error: te }, { data: tmD, error: tme }] = await Promise.all([
      ...
      if (te) throw te; if (tme) throw tme;
      ...
      if (code) {
        ...
        if (found) {
          ...
          setInviteInfo({ teamId: found.id, teamName: found.name });
        }
  ...
  async function register() {
    ...
    const duplicate = Object.values(users).some(u => u.name.toLowerCase() === newName.trim().toLowerCase());
  ...
  async function createTeam() {
    ...
    const team = { id, name: newTeamName.trim(), created_by: uid, invite_code };
    const { error: e } = await supabase.from("teams").insert(team);
    ...
    setTeams(t => ({ ...t, [id]: { name: team.name, created_by: uid, invite_code } }));
    setMembers(m => ({ ...m, [id]: [uid] }));
    setNewTeamName(""); setActiveTeam(id); setTeamView("detail"); setSaving(false);
  }

  // ── týmy: vstup přes pozvánku ────────────────────────────────────────────
  ...
  function buildTeamLB(teamId) {
    const t = todayStr(), w = weekAgoStr();
    const teamMemberIds = members[teamId] || [];
    const result = {};
    for (const id of teamMemberIds) {
      ...
      for (const a of ACTIVITY_META) acts[a.key] = 0;
      for (const [date, e] of Object.entries(days)) {
        if (teamPeriod === "today" && date !== t) continue;
        if (teamPeriod === "week"  && date < w)  continue;
        sc += calcScore(e, calcAge(u.dob), pts);
        for (const a of ACTIVITY_META) acts[a.key] += parseFloat(e[a.key]) || 0;
      }
      result[id] = { sc, name: u.name, age: calcAge(u.dob), acts };
    ...
    <div style={P}>
      ...
      {inviteInfo && (
        <div style={{background:"var(--color-background-success)",border:"0.5px solid var(--color-border-success)",borderRadius:"var(--border-radius-md)",padding:"12px 14px",marginBottom:"1rem"}}>
          ...
          <p style={{margin:"0 0 10px",fontSize:13,color:"var(--color-text-success)"}}>Byl jsi pozván do týmu <strong>{inviteInfo.teamName}</strong>. Přihlas se a automaticky se připojíš.</p>
        </div>
      )}
  ...
  if (view === "admin") return (
    <div style={P}>
      <style>{FONT}</style>
      <div style={topBarStyle}>
        <div><p style={{margin:0,fontWeight:700,fontSize:15,fontFamily:"'DM Sans',sans-serif",color:"var(--color-text-primary)"}}>Administrátor</p></div>
          ...
          {ACTIVITY_META.map(a=>{
            ...
            return (
              <div key={a.key} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"var(--color-background-primary)",border:`0.5px solid ${changed?"var(--color-border-warning)":"var(--color-border-tertiary)"}`,borderRadius:"var(--border-radius-md)"}}>
                ...
                <input type="number" min="0" step="0.01" value={current} onChange={e=>setPtsEdit(p=>({...p,[a.key]:parseFloat(e.target.value)||0}))}
                  style={{width:70,textAlign:"right",padding:"7px 8px",fontSize:14,background:"var(--color-background-secondary)",border:`0.5px solid ${changed?"var(--color-border-warning)":"var(--color-border-tertiary)"}`,borderRadius:"var(--border-radius-md)",color:"var(--color-text-primary)",outline:"none",fontFamily:"'DM Mono',monospace"}}/>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {ACTIVITIES.map(a=>(
            <div key={a.key} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)"}}>
              ...
              <input type="number" min="0" step={a.unit==="km"?"0.1":"1"} value={form[a.key]||""} placeholder="—"
  ...
  if (view === "teams") {
    ...
    if (teamView === "list") return (
      <div style={P}>
        ...
        {myTeamIds.length===0&&<p style={{fontSize:13,color:"var(--color-text-secondary)"}}>Zatím nejsi v žádném týmu. Vytvoř nový nebo se připoj přes pozvánku.</p>}
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {myTeamIds.map(tid=>{
            const t=teams[tid]; if(!t) return null;
            const mCount=(members[tid]||[]).length;
            return (
              <button key={tid} onClick={()=>{setActiveTeam(tid);setTeamView("detail");setTeamTab("score");}}
                ...
                <div style={{flex:1}}>
                  ...
                  <p style={{margin:0,fontSize:11,color:"var(--color-text-secondary)"}}>{mCount} {mCount===1?"člen":"členů"}{t.created_by===uid?" · tvůj tým":""}</p>
                </div>
    ...
    if (teamView === "create") return (
      <div style={P}>
        ...
        <div style={card}>
          <p style={{...SL,marginBottom:10}}>Název týmu</p>
          <input placeholder="Např. Běžci z 3. patra" value={newTeamName} onChange={e=>setNewTeamName(e.target.value)}
            style={{...inputStyle,marginBottom:12}} onKeyDown={e=>e.key==="Enter"&&createTeam()}/>
          <button onClick={createTeam} disabled={!newTeamName.trim()||saving} style={primaryBtn}>
            {saving?"Vytvářím…":"Vytvořit tým →"}
          </button>
        </div>
      </div>
    );
    ...
    if (teamView === "detail" && activeTeam) {
      ...
      const lb = buildTeamLB(activeTeam);
      const sorted = Object.entries(lb).sort((a,b)=>b[1].sc-a[1].sc);
      const myRank = sorted.findIndex(([id])=>id===uid)+1;
      ...
      return (
        <div style={P}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:"1rem"}}>
            <button onClick={()=>setTeamView("list")} style={{fontSize:20,background:"none",border:"none",cursor:"pointer",color:"var(--color-text-secondary)",padding:0}}>‹</button>
            <div style={{flex:1}}>
              <p style={{margin:0,fontSize:16,fontWeight:700,fontFamily:"'DM Sans',sans-serif",color:"var(--color-text-primary)"}}>{team.name}</p>
              <p style={{margin:0,fontSize:11,color:"var(--color-text-secondary)"}}>{(members[activeTeam]||[]).length} členů</p>
            </div>
          ...
          <div style={{...card,marginBottom:"1rem"}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              <button onClick={()=>{navigator.clipboard.writeText(inviteUrl);}}
                style={{padding:"9px 14px",fontWeight:700,fontSize:12,background:"var(--color-text-primary)",color:"var(--color-background-primary)",border:"none",borderRadius:"var(--border-radius-md)",cursor:"pointer",flexShrink:0,whiteSpace:"nowrap"}}>
                Kopírovat
              </button>
            </div>
            <p style={{margin:"6px 0 0",fontSize:11,color:"var(--color-text-secondary)"}}>Pošli odkaz kolegovi — po kliknutí se automaticky připojí.</p>
          </div>
          ...
          {myRank>0&&<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"var(--color-background-secondary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",padding:"10px 14px",marginBottom:"1rem"}}>
            <span style={{fontSize:12,color:"var(--color-text-secondary)"}}>Tvoje pořadí v týmu</span>
            <span style={{fontSize:24,fontWeight:700,fontFamily:"'DM Mono',monospace",color:"var(--color-text-primary)"}}># {myRank}</span>
            <span style={{fontSize:12,color:"var(--color-text-secondary)"}}>z {sorted.length}</span>
          </div>}

          {/* záložky body / aktivity */}
                    ...
                    {vals.filter(v=>v.val>0).map((v,i)=>(
                      <div key={v.name}>
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                          <span style={{fontSize:12,fontWeight:v.isMe?700:400,color:v.isMe?"var(--color-text-primary)":"var(--color-text-secondary)",fontFamily:"'DM Sans',sans-serif"}}>
                            {i===0?"🥇 ":""}{v.name}
                          </span>
                          <span style={{fontSize:12,fontFamily:"'DM Mono',monospace",color:"var(--color-text-primary)",fontWeight:700}}>
                            {a.unit==="km"?v.val.toFixed(1):Math.round(v.val)} {a.unit}
                          </span>
                        </div>
        ...
        <div style={{...card,marginBottom:"1rem",padding:"1rem"}}>
          <div style={{display:"flex",alignItems:"flex-end",gap:3,height:64}}>
            {chartDays.map((d,i)=>{
              ...
              const h=sc>0?Math.max(4,Math.round((sc/maxSc)*56)):2;
              return <div key={d} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center"}}>
                <div title={`${d}: ${sc.toFixed(1)} b`} style={{width:"100%",height:h,background:d===todayStr()?"#38bdf8":sc>0?"#c084fc":"var(--color-border-tertiary)",borderRadius:"3px 3px 0 0",cursor:"default"}}/>
              </div>;
            })}
          </div>
