...
function calcScore(e, age, pts) {
  if (!e) return 0;
  return getActivities(pts).reduce((s, a) => s + (parseFloat(e[a.key]) || 0) * a.pts, 0) * ageMult(age);
}
...
function exportCSV(userName, days, pts, age) {
  ...
  a.href=url; a.download=`beatfit_${userName}_${todayStr()}.csv`; a.click();
...
function daysLeft(s) {
  const diff = Math.ceil((new Date(s.end_date)-new Date(todayStr()))/(1000*60*60*24));
  return diff;
}
...
export default function App() {
  ...
  const [seasons, setSeasons]     = useState({});   // id -> season obj
  ...
  // season state
  const [lbMode, setLbMode]       = useState("global");   // global | season:<id>
  const [teamSeasonMode, setTeamSeasonMode] = useState("all"); // all | season:<id>
  ...
  const [showTeamSeasonForm, setShowTeamSeasonForm] = useState(false);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [
        {data:uD,error:ue},{data:eD,error:ee},{data:sD},{data:gD},
        {data:tD,error:te},{data:tmD,error:tme},{data:seD,error:see}
      ] = await Promise.all([
      ...
      if(ue)throw ue; if(ee)throw ee; if(te)throw te; if(tme)throw tme; if(see)throw see;
      ...
      if(sD?.length>0) setPts({...DEFAULT_PTS,...sD[0].value});
      ...
      const teamsMap={};
      for(const t of(tD||[])) teamsMap[t.id]={name:t.name,created_by:t.created_by,invite_code:t.invite_code};
      setTeams(teamsMap);

      const membersMap={};
      for(const m of(tmD||[])){
        if(!membersMap[m.team_id]) membersMap[m.team_id]=[];
        membersMap[m.team_id].push(m.user_id);
      }
      setMembers(membersMap);

      const seasonsMap={};
      for(const s of(seD||[])) seasonsMap[s.id]={
        name:s.name, start_date:s.start_date, end_date:s.end_date,
        created_by:s.created_by, team_id:s.team_id, scope:s.scope
      };
      ...
      if(code){
        const found=tD?.find(t=>t.invite_code===code);
        if(found){setPendingInvite(code);setInviteInfo({teamId:found.id,teamName:found.name});}
  ...
  async function saveEditUser(){
    ...
    const dup=Object.entries(users).some(([id,u])=>id!==editUser&&u.name.toLowerCase()===editName.trim().toLowerCase());
  ...
  async function deleteUser(id){
    ...
    await Promise.all([
      supabase.from("entries").delete().eq("user_id",id),
      supabase.from("goals").delete().eq("user_id",id),
      supabase.from("team_members").delete().eq("user_id",id),
    ]);
  ...
  async function createSeason(teamId=null){
    ...
    const id="s"+Date.now();
    const newSeason={
      id, name:seasonForm.name.trim(),
      start_date:seasonForm.start_date, end_date:seasonForm.end_date,
      created_by:uid, team_id:teamId||null,
      scope:teamId?"team":"global"
    };
    const{error:e}=await supabase.from("seasons").insert(newSeason);
    if(e){setError("Vytvoření sezóny selhalo.");setSeasonSaving(false);return;}
    setSeasons(s=>({...s,[id]:{name:newSeason.name,start_date:newSeason.start_date,end_date:newSeason.end_date,created_by:uid,team_id:teamId||null,scope:newSeason.scope}}));
    setSeasonForm({name:"",start_date:"",end_date:""});
    setShowSeasonForm(false);setShowTeamSeasonForm(false);
    setSeasonFlash(true);setTimeout(()=>setSeasonFlash(false),2000);
  ...
  async function createTeam(){
    ...
    const{error:e}=await supabase.from("teams").insert({id,name:newTeamName.trim(),created_by:uid,invite_code});
  ...
  async function joinTeamByCode(code){
    ...
    if(!team){setError("Pozvánka nebyla nalezena.");return;}
    const[teamId]=team;
    const alreadyIn=(members[teamId]||[]).includes(uid);
    if(alreadyIn){setPendingInvite(null);setInviteInfo(null);setActiveTeam(teamId);setTeamView("detail");setView("teams");return;}
    ...
    setPendingInvite(null);setInviteInfo(null);
    setActiveTeam(teamId);setTeamView("detail");setView("teams");
  }
  ...
  function navBtn(active){return{flex:1,fontSize:12,fontWeight:500,border:"none",cursor:"pointer",padding:"7px 0",borderRadius:6,background:active?"var(--color-background-primary)":"transparent",color:active?"var(--color-text-primary)":"var(--color-text-secondary)",boxShadow:active?"0 0 0 0.5px var(--color-border-tertiary)":"none"};}
  function chipBtn(active){return{flex:1,fontSize:12,fontWeight:500,cursor:"pointer",padding:"6px 0",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",background:active?"var(--color-text-primary)":"transparent",color:active?"var(--color-background-primary)":"var(--color-text-secondary)"};}
  ...
  const primaryBtn={display:"block",width:"100%",padding:"11px",fontWeight:700,fontSize:14,background:"var(--color-text-primary)",color:"var(--color-background-primary)",border:"none",borderRadius:"var(--border-radius-md)",cursor:"pointer",letterSpacing:0.3};
  const inputStyle={display:"block",width:"100%",boxSizing:"border-box",padding:"9px 12px",fontSize:14,background:"var(--color-background-secondary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",color:"var(--color-text-primary)",outline:"none",fontFamily:"'DM Sans',sans-serif"};
  ...
  // season form reusable
  const SeasonFormCard=({onSubmit,onCancel})=>(
    <div style={{...card,marginBottom:"1rem"}}>
      <p style={{...SL,marginBottom:10}}>Nová výzva</p>
      <input placeholder="Název výzvy" value={seasonForm.name} onChange={e=>setSeasonForm(f=>({...f,name:e.target.value}))} style={{...inputStyle,marginBottom:8}}/>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
        <div>
          <label style={{display:"block",fontSize:11,color:"var(--color-text-secondary)",marginBottom:4,fontWeight:600}}>Začátek</label>
          <input type="date" value={seasonForm.start_date} onChange={e=>setSeasonForm(f=>({...f,start_date:e.target.value}))} style={{...inputStyle,fontFamily:"'DM Mono',monospace"}}/>
        </div>
        <div>
          <label style={{display:"block",fontSize:11,color:"var(--color-text-secondary)",marginBottom:4,fontWeight:600}}>Konec</label>
          <input type="date" value={seasonForm.end_date} onChange={e=>setSeasonForm(f=>({...f,end_date:e.target.value}))} style={{...inputStyle,fontFamily:"'DM Mono',monospace"}}/>
        </div>
      </div>
      <div style={{display:"flex",gap:8}}>
        <button onClick={onSubmit} disabled={!seasonForm.name.trim()||!seasonForm.start_date||!seasonForm.end_date||seasonSaving} style={{...primaryBtn,flex:1}}>
          {seasonSaving?"Vytvářím…":seasonFlash?"✓ Vytvořeno!":"Vytvořit výzvu →"}
        </button>
  ...
  // season card reusable
  const SeasonCard=({s,sid,showDelete})=>{
        ...
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}>
          <div style={{flex:1}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
              <span style={{fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:20,background:lbl.bg,color:lbl.color}}>{lbl.text}</span>
              {st==="active"&&dl>=0&&<span style={{fontSize:11,color:"var(--color-text-secondary)"}}>zbývá {dl} {dl===1?"den":"dní"}</span>}
            </div>
            <p style={{margin:"0 0 2px",fontSize:14,fontWeight:700,fontFamily:"'DM Sans',sans-serif",color:"var(--color-text-primary)"}}>{s.name}</p>
            <p style={{margin:0,fontSize:11,fontFamily:"'DM Mono',monospace",color:"var(--color-text-secondary)"}}>{s.start_date} → {s.end_date}</p>
          </div>
          {showDelete&&(
            <button onClick={()=>deleteSeason(sid)} style={{fontSize:11,padding:"4px 10px",background:"transparent",border:"0.5px solid var(--color-border-danger)",borderRadius:6,cursor:"pointer",color:"var(--color-text-danger)",flexShrink:0}}>Smazat</button>
          )}
        </div>
      </div>
    );
  ...
  if(view==="login") return(
    <div style={P}>
      ...
      <ErrorBanner/>
      {inviteInfo&&(
        <div style={{background:"var(--color-background-success)",border:"0.5px solid var(--color-border-success)",borderRadius:"var(--border-radius-md)",padding:"12px 14px",marginBottom:"1rem"}}>
          <p style={{margin:"0 0 4px",fontWeight:700,fontSize:14,color:"var(--color-text-success)"}}>Pozvánka do týmu</p>
          <p style={{margin:0,fontSize:13,color:"var(--color-text-success)"}}>Byl jsi pozván do týmu <strong>{inviteInfo.teamName}</strong>. Přihlas se a automaticky se připojíš.</p>
        </div>
      )}
      {Object.keys(users).length>0&&(
        <div style={{marginBottom:"1.5rem"}}>
          <p style={SL}>Přihlásit se jako</p>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {Object.entries(users).sort(([,a],[,b])=>a.name.localeCompare(b.name)).map(([id,u])=>{
              const streak=calcStreak(entries[id]||{});
              return(
                <button key={id} onClick={async()=>{loginUser(id);if(pendingInvite) await joinTeamByCode(pendingInvite);}}
                  style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"var(--color-background-secondary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",cursor:"pointer",width:"100%"}}>
      ...
      <div style={card}>
        ...
        <button onClick={register} disabled={!newName.trim()||!newDob||saving} style={primaryBtn}>{saving?"Registruji…":"Zaregistrovat se →"}</button>
      </div>
      <div style={{...card,marginTop:"1rem"}}>
        <p style={{...SL,marginBottom:10}}>Administrátor</p>
        <input type="password" placeholder="Heslo" value={adminPwd} onChange={e=>{setAdminPwd(e.target.value);setAdminError(false);}} onKeyDown={e=>e.key==="Enter"&&loginAdmin()} style={{...inputStyle,marginBottom:8,borderColor:adminError?"var(--color-border-danger)":"var(--color-border-tertiary)"}}/>
  ...
  const Nav=()=>(
    <div style={navWrap}>
      {[["log","Záznam"],["leaderboard","Žebříček"],["teams","Týmy"],["stats","Moje"]].map(([v,l])=>(
        <button key={v} onClick={()=>{setView(v);if(v==="teams")setTeamView("list");}} style={navBtn(view===v)}>{l}</button>
      ))}
    </div>
  );
  ...
  if(view==="admin") return(
    <div style={P}>
      ...
      <div style={{...navWrap,marginBottom:"1rem"}}>
        {[["pts","Koeficienty"],["seasons","Výzvy"],["players","Hráči"]].map(([t,l])=>(
          <button key={t} onClick={()=>{setAdminTab(t);setEditUser(null);setError(null);setShowSeasonForm(false);}} style={navBtn(adminTab===t)}>{l}</button>
        ))}
      </div>
      ...
      {adminTab==="pts"&&<>
        ...
        <button onClick={()=>setPtsEdit({...DEFAULT_PTS})} style={{...primaryBtn,background:"var(--color-background-secondary)",color:"var(--color-text-secondary)",border:"0.5px solid var(--color-border-tertiary)",marginBottom:8}}>Obnovit výchozí hodnoty</button>
        <button onClick={savePts} disabled={ptsSaving} style={primaryBtn}>{ptsSaving?"Ukládám…":ptsFlash?"✓ Uloženo!":"Uložit koeficienty"}</button>
      </>}

      {adminTab==="seasons"&&<>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1rem"}}>
          <p style={{...SL,margin:0}}>Globální výzvy</p>
          {!showSeasonForm&&<button onClick={()=>{setShowSeasonForm(true);setSeasonForm({name:"",start_date:"",end_date:""});}} style={{fontSize:12,fontWeight:700,padding:"5px 14px",background:"var(--color-text-primary)",color:"var(--color-background-primary)",border:"none",borderRadius:20,cursor:"pointer"}}>+ Nová výzva</button>}
        </div>
        {showSeasonForm&&<SeasonFormCard onSubmit={()=>createSeason(null)} onCancel={()=>setShowSeasonForm(false)}/>}
        {globalSeasons.length===0&&!showSeasonForm&&<p style={{fontSize:13,color:"var(--color-text-secondary)",marginBottom:"1rem"}}>Zatím žádné výzvy.</p>}
        {globalSeasons.sort(([,a],[,b])=>b.start_date.localeCompare(a.start_date)).map(([sid,s])=>(
          <SeasonCard key={sid} s={s} sid={sid} showDelete={true}/>
        ))}
      </>}

      {adminTab==="players"&&<>
        <p style={SL}>Správa hráčů</p>
        {editUser?(
          <div style={card}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:5}}>
            {Object.entries(users).sort(([,a],[,b])=>a.name.localeCompare(b.name)).map(([id,u])=>(
              <div key={id} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)"}}>
                ...
                <div style={{flex:1}}><p style={{margin:0,fontSize:13,fontWeight:700,color:"var(--color-text-primary)",fontFamily:"'DM Sans',sans-serif"}}>{u.name}</p><p style={{margin:0,fontSize:10,color:"var(--color-text-secondary)"}}>{calcAge(u.dob)} let · {Object.keys(entries[id]||{}).length} záznamů</p></div>
                <button onClick={()=>{setEditUser(id);setEditName(u.name);setEditDob(u.dob);setError(null);}} style={{fontSize:11,padding:"4px 10px",background:"var(--color-background-secondary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:6,cursor:"pointer",color:"var(--color-text-secondary)"}}>Upravit</button>
                <button onClick={()=>deleteUser(id)} style={{fontSize:11,padding:"4px 10px",background:"transparent",border:"0.5px solid var(--color-border-danger)",borderRadius:6,cursor:"pointer",color:"var(--color-text-danger)"}}>Smazat</button>
              </div>
            ))}
          </div>
        )}
  ...
  // ── záznam ───────────────────────────────────────────────────────────────
  if(view==="log"){
    const age=calcAge(user.dob), sc=calcScore(form,age,pts), streak=calcStreak(entries[uid]||{});
    ...
    return(
      <div style={P}>
        ...
        <TopBar/><Nav/>
        <ErrorBanner/>
        <input type="date" value={logDate} max={todayStr()} onChange={e=>{setLogDate(e.target.value);setForm(entries[uid]?.[e.target.value]||{});}} style={{...inputStyle,marginBottom:"1rem",fontFamily:"'DM Mono',monospace"}}/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:"1rem"}}>
          <div style={statCard}><p style={{margin:"0 0 4px",fontSize:10,fontWeight:700,letterSpacing:0.5,textTransform:"uppercase",color:"var(--color-text-secondary)"}}>Skóre</p><p style={{margin:0,fontSize:22,fontWeight:700,fontFamily:"'DM Mono',monospace",color:"var(--color-text-primary)"}}>{sc.toFixed(1)}</p></div>
          <div style={statCard}><p style={{margin:"0 0 4px",fontSize:10,fontWeight:700,letterSpacing:0.5,textTransform:"uppercase",color:"var(--color-text-secondary)"}}>Věk. koef.</p><p style={{margin:0,fontSize:22,fontWeight:700,fontFamily:"'DM Mono',monospace",color:"var(--color-text-primary)"}}>×{ageMult(age).toFixed(2)}</p></div>
          <div style={statCard}><p style={{margin:"0 0 4px",fontSize:10,fontWeight:700,letterSpacing:0.5,textTransform:"uppercase",color:"var(--color-text-secondary)"}}>Streak</p><p style={{margin:0,fontSize:22,fontWeight:700,fontFamily:"'DM Mono',monospace",color:streak>=7?"#f97316":"var(--color-text-primary)"}}>{streak}{streak>0?" 🔥":""}</p></div>
        </div>
        {weekGoal>0&&<div style={{...statCard,marginBottom:"1rem"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><p style={{margin:0,fontSize:11,fontWeight:700,letterSpacing:0.5,textTransform:"uppercase",color:"var(--color-text-secondary)"}}>Týdenní cíl</p><span style={{fontSize:12,fontFamily:"'DM Mono',monospace",color:"var(--color-text-secondary)"}}>{weekScore.toFixed(0)} / {weekGoal} b</span></div><div style={{height:6,background:"var(--color-border-tertiary)",borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:`${goalPct}%`,background:goalPct>=100?"#34d399":"#38bdf8",borderRadius:3}}/></div>{goalPct>=100&&<p style={{margin:"6px 0 0",fontSize:11,color:"#34d399",fontWeight:700}}>Cíl splněn! 🎉</p>}</div>}
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {ACTIVITIES.map(a=>(
        ...
        <div style={{display:"flex",gap:8,marginTop:"1rem"}}>
          <button onClick={saveEntry} disabled={saving} style={{...primaryBtn,flex:1}}>{saving?"Ukládám…":flash?"✓ Uloženo!":"Uložit výkon"}</button>
  ...
  if(view==="leaderboard"){
    ...
    const activeSeason = lbMode.startsWith("season:") ? seasons[lbMode.slice(7)] : null;
    const lb = activeSeason
      ? buildLB(null, activeSeason.start_date, activeSeason.end_date)
      : buildLB();
    ...
    return(
      <div style={P}>
        ...
        {/* mode selector: global periods OR season */}
        {lbMode==="global"?(
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:"1rem"}}>
            ...
            <div style={{flex:1}}>
              <p style={{margin:0,fontSize:14,fontWeight:700,color:"var(--color-text-primary)",fontFamily:"'DM Sans',sans-serif"}}>{activeSeason?.name}</p>
              <p style={{margin:0,fontSize:11,fontFamily:"'DM Mono',monospace",color:"var(--color-text-secondary)"}}>{activeSeason?.start_date} → {activeSeason?.end_date}</p>
            </div>
            <span style={{fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:20,...(activeSeason?seasonLabel(activeSeason):{})}}>{activeSeason?seasonLabel(activeSeason).text:""}</span>
          </div>
        )}
          ...
          <div style={{marginBottom:"1rem"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              {globalSeasons.sort(([,a],[,b])=>b.start_date.localeCompare(a.start_date)).map(([sid,s])=>{
                ...
                return(
                  <button key={sid} onClick={()=>setLbMode(`season:${sid}`)}
                    style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",cursor:"pointer",width:"100%",textAlign:"left"}}>
                    <div style={{flex:1}}>
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                        <span style={{fontSize:10,fontWeight:700,padding:"1px 7px",borderRadius:20,background:lbl.bg,color:lbl.color}}>{lbl.text}</span>
                        {st==="active"&&dl>=0&&<span style={{fontSize:10,color:"var(--color-text-secondary)"}}>zbývá {dl} dní</span>}
                      </div>
                      <p style={{margin:0,fontSize:13,fontWeight:700,fontFamily:"'DM Sans',sans-serif",color:"var(--color-text-primary)"}}>{s.name}</p>
                      <p style={{margin:0,fontSize:10,fontFamily:"'DM Mono',monospace",color:"var(--color-text-secondary)"}}>{s.start_date} → {s.end_date}</p>
                    </div>
        ...
        {myRank>0&&<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"var(--color-background-secondary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",padding:"10px 14px",marginBottom:"1rem"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:5,marginBottom:"1.5rem"}}>
          ...
          {sorted.map(([id,d],i)=>{
            const streak=calcStreak(entries[id]||{});
            return<div key={id} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",background:id===uid?"var(--color-background-info)":"var(--color-background-secondary)",border:`0.5px solid ${id===uid?"var(--color-border-info)":"transparent"}`,borderRadius:"var(--border-radius-md)"}}>
              <span style={{fontSize:16,minWidth:28,color:RANK_CLR[i]||"var(--color-text-secondary)",fontWeight:700}}>{MEDALS[i]||`${i+1}.`}</span>
              <div style={{width:26,height:26,borderRadius:"50%",background:"var(--color-background-info)",color:"var(--color-text-info)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,flexShrink:0}}>{d.name[0]}</div>
  ...
  if(view==="teams"){
    ...
    if(teamView==="list") return(
      <div style={P}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {myTeamIds.map(tid=>{
            const t=teams[tid];if(!t)return null;
            const mc=(members[tid]||[]).length;
            const teamSeasonCount=Object.values(seasons).filter(s=>s.team_id===tid).length;
            return(
              <button key={tid} onClick={()=>{setActiveTeam(tid);setTeamView("detail");setTeamTab("score");setTeamSeasonMode("all");setShowTeamSeasonForm(false);}}
                style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",cursor:"pointer",width:"100%",textAlign:"left"}}>
                ...
                <div style={{flex:1}}>
                  <p style={{margin:0,fontSize:14,fontWeight:700,fontFamily:"'DM Sans',sans-serif",color:"var(--color-text-primary)"}}>{t.name}</p>
                  <p style={{margin:0,fontSize:11,color:"var(--color-text-secondary)"}}>{mc} {mc===1?"člen":"členů"}{teamSeasonCount>0?` · ${teamSeasonCount} výzev`:""}{t.created_by===uid?" · tvůj tým":""}</p>
                </div>
                <span style={{fontSize:18,color:"var(--color-text-secondary)"}}>›</span>
              </button>
            );
    ...
    if(teamView==="create") return(
      <div style={P}>
        ...
        <div style={card}>
          <input placeholder="Název týmu" value={newTeamName} onChange={e=>setNewTeamName(e.target.value)} style={{...inputStyle,marginBottom:12}} onKeyDown={e=>e.key==="Enter"&&createTeam()}/>
          <button onClick={createTeam} disabled={!newTeamName.trim()||saving} style={primaryBtn}>{saving?"Vytvářím…":"Vytvořit tým →"}</button>
        </div>
      </div>
    );
    ...
    if(teamView==="detail"&&activeTeam){
      ...
      const activeSeason=teamSeasonMode.startsWith("season:")
        ?seasons[teamSeasonMode.slice(7)] : null;

      const lb=activeSeason
        ? (() => {
            const t2=todayStr(); const w2=weekAgoStr();
            const memberIds=members[activeTeam]||[];
            const result={};
            for(const id of memberIds){
              ...
              for(const a of ACTIVITY_META) acts[a.key]=0;
              for(const[date,e] of Object.entries(days)){
                if(date<activeSeason.start_date||date>activeSeason.end_date) continue;
                sc+=calcScore(e,calcAge(u.dob),pts);
                for(const a of ACTIVITY_META) acts[a.key]+=parseFloat(e[a.key])||0;
              }
              result[id]={sc,name:u.name,age:calcAge(u.dob),acts};
        ...
        : (() => {
            ...
            for(const id of memberIds){
              ...
              for(const a of ACTIVITY_META) acts[a.key]=0;
              for(const[date,e] of Object.entries(days)){
                if(teamPeriod==="today"&&date!==t2) continue;
                if(teamPeriod==="week"&&date<w2) continue;
                sc+=calcScore(e,calcAge(u.dob),pts);
                for(const a of ACTIVITY_META) acts[a.key]+=parseFloat(e[a.key])||0;
              }
              result[id]={sc,name:u.name,age:calcAge(u.dob),acts};
      ...
      const sorted=Object.entries(lb).sort((a,b)=>b[1].sc-a[1].sc);
      const myRank=sorted.findIndex(([id])=>id===uid)+1;
      const inviteUrl=`${window.location.origin}${window.location.pathname}?invite=${team.invite_code}`;
      ...
      return(
        <div style={P}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:"1rem"}}>
            <button onClick={()=>setTeamView("list")} style={{fontSize:20,background:"none",border:"none",cursor:"pointer",color:"var(--color-text-secondary)",padding:0}}>‹</button>
            <div style={{flex:1}}><p style={{margin:0,fontSize:16,fontWeight:700,fontFamily:"'DM Sans',sans-serif",color:"var(--color-text-primary)"}}>{team.name}</p><p style={{margin:0,fontSize:11,color:"var(--color-text-secondary)"}}>{(members[activeTeam]||[]).length} členů</p></div>
          ...
          <div style={{...card,marginBottom:"1rem"}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <input readOnly value={inviteUrl} style={{...inputStyle,flex:1,fontSize:11,fontFamily:"'DM Mono',monospace",cursor:"text"}}/>
          ...
          <div style={{marginBottom:"1rem"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
              ...
              {!showTeamSeasonForm&&<button onClick={()=>{setShowTeamSeasonForm(true);setSeasonForm({name:"",start_date:"",end_date:""}); }} style={{fontSize:11,fontWeight:700,padding:"4px 12px",background:"var(--color-text-primary)",color:"var(--color-background-primary)",border:"none",borderRadius:20,cursor:"pointer"}}>+ Nová výzva</button>}
            </div>
            {showTeamSeasonForm&&<SeasonFormCard onSubmit={()=>createSeason(activeTeam)} onCancel={()=>setShowTeamSeasonForm(false)}/>}

            {/* mode chips: all + seasons */}
            <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
              <button onClick={()=>setTeamSeasonMode("all")} style={{...chipBtn(teamSeasonMode==="all"),flex:"none",padding:"5px 12px",fontSize:11}}>Bez výzvy</button>
              {teamSeasonsList.sort(([,a],[,b])=>b.start_date.localeCompare(a.start_date)).map(([sid,s])=>{
                const lbl=seasonLabel(s);
                return<button key={sid} onClick={()=>setTeamSeasonMode(`season:${sid}`)}
                  style={{...chipBtn(teamSeasonMode===`season:${sid}`),flex:"none",padding:"5px 12px",fontSize:11,display:"flex",alignItems:"center",gap:5}}>
                  <span style={{width:6,height:6,borderRadius:"50%",background:lbl.color,flexShrink:0,display:"inline-block"}}/>
                  {s.name}
                </button>;
              })}
            </div>

            {/* delete season button when viewing one */}
            {activeSeason&&(members[activeTeam]||[]).includes(uid)&&activeSeason.created_by===uid&&(
              <button onClick={()=>{deleteSeason(teamSeasonMode.slice(7));setTeamSeasonMode("all");}} style={{marginTop:6,fontSize:11,padding:"3px 10px",background:"transparent",border:"0.5px solid var(--color-border-danger)",borderRadius:6,cursor:"pointer",color:"var(--color-text-danger)"}}>Smazat tuto výzvu</button>
            )}
          </div>

          {/* period filter (only in "all" mode) */}
          {teamSeasonMode==="all"&&(
            <div style={{display:"flex",gap:4,marginBottom:"1rem"}}>
              {[["today","Dnes"],["week","Týden"],["all","Vše"]].map(([k,l])=>(
                <button key={k} onClick={()=>setTeamPeriod(k)} style={chipBtn(teamPeriod===k)}>{l}</button>
              ))}
            </div>
          )}
          ...
          {activeSeason&&(
            <div style={{...card,marginBottom:"1rem",background:"var(--color-background-secondary)"}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:20,...seasonLabel(activeSeason)}}>{seasonLabel(activeSeason).text}</span>
                <div style={{flex:1}}><p style={{margin:0,fontSize:13,fontWeight:700,color:"var(--color-text-primary)",fontFamily:"'DM Sans',sans-serif"}}>{activeSeason.name}</p><p style={{margin:0,fontSize:10,fontFamily:"'DM Mono',monospace",color:"var(--color-text-secondary)"}}>{activeSeason.start_date} → {activeSeason.end_date}</p></div>
                {seasonStatus(activeSeason)==="active"&&<span style={{fontSize:11,color:"var(--color-text-secondary)"}}>zbývá {daysLeft(activeSeason)} dní</span>}
              </div>
            </div>
          )}
          ...
          {myRank>0&&<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"var(--color-background-secondary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)",padding:"10px 14px",marginBottom:"1rem"}}>
            <span style={{fontSize:12,color:"var(--color-text-secondary)"}}>Tvoje pořadí</span>
            <span style={{fontSize:24,fontWeight:700,fontFamily:"'DM Mono',monospace",color:"var(--color-text-primary)"}}># {myRank}</span>
            <span style={{fontSize:12,color:"var(--color-text-secondary)"}}>z {sorted.length}</span>
          </div>}
          ...
          <div style={{display:"flex",gap:3,background:"var(--color-background-secondary)",padding:3,borderRadius:"var(--border-radius-md)",marginBottom:"1rem"}}>
            {[["score","Body"],["activity","Aktivity"]].map(([t,l])=>(
              <button key={t} onClick={()=>setTeamTab(t)} style={navBtn(teamTab===t)}>{l}</button>
            ))}
          </div>

          {teamTab==="score"&&<>
            <p style={SL}>Žebříček bodů</p>
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              {sorted.length===0&&<p style={{fontSize:13,color:"var(--color-text-secondary)"}}>Zatím žádná data.</p>}
              {sorted.map(([id,d],i)=>(
          ...
          {teamTab==="activity"&&<>
            <p style={SL}>Porovnání aktivit</p>
            {ACTIVITY_META.map(a=>{
              ...
              return(
                <div key={a.key} style={{...card,marginBottom:"0.75rem"}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                    <div style={{width:28,height:28,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,background:a.color+"22",color:a.color,flexShrink:0}}>{a.icon}</div>
                    ...
                    {vals.filter(v=>v.val>0).map((v,i)=>(
                      <div key={v.name}>
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                          <span style={{fontSize:12,fontWeight:v.isMe?700:400,color:v.isMe?"var(--color-text-primary)":"var(--color-text-secondary)",fontFamily:"'DM Sans',sans-serif"}}>{i===0?"🥇 ":""}{v.name}</span>
                          <span style={{fontSize:12,fontFamily:"'DM Mono',monospace",color:"var(--color-text-primary)",fontWeight:700}}>{a.unit==="km"?v.val.toFixed(1):Math.round(v.val)} {a.unit}</span>
                        </div>
                        <div style={{height:5,background:"var(--color-border-tertiary)",borderRadius:3,overflow:"hidden"}}>
                          <div style={{height:"100%",width:`${(v.val/maxVal)*100}%`,background:v.isMe?a.color:"var(--color-text-secondary)",borderRadius:3,opacity:v.isMe?1:0.4}}/>
                        </div>
                      </div>
                    ))}
                  </div>
    ...
    return(
      <div style={P}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:"1rem"}}>
          <div style={statCard}><p style={{margin:"0 0 3px",fontSize:10,fontWeight:700,letterSpacing:0.5,textTransform:"uppercase",color:"var(--color-text-secondary)"}}>Celkem bodů</p><p style={{margin:0,fontSize:20,fontWeight:700,fontFamily:"'DM Mono',monospace",color:"var(--color-text-primary)"}}>{total.toFixed(0)}</p></div>
          <div style={statCard}><p style={{margin:"0 0 3px",fontSize:10,fontWeight:700,letterSpacing:0.5,textTransform:"uppercase",color:"var(--color-text-secondary)"}}>Aktivní dny</p><p style={{margin:0,fontSize:20,fontWeight:700,fontFamily:"'DM Mono',monospace",color:"var(--color-text-primary)"}}>{dates.length}</p></div>
          <div style={statCard}><p style={{margin:"0 0 3px",fontSize:10,fontWeight:700,letterSpacing:0.5,textTransform:"uppercase",color:"var(--color-text-secondary)"}}>Streak</p><p style={{margin:0,fontSize:20,fontWeight:700,fontFamily:"'DM Mono',monospace",color:streak>=7?"#f97316":"var(--color-text-primary)"}}>{streak}{streak>0?" 🔥":""}</p></div>
        </div>
        <div style={{...card,marginBottom:"1rem"}}>
          <p style={{...SL,marginBottom:8}}>Týdenní cíl (body)</p>
          <div style={{display:"flex",gap:8,marginBottom:weekGoal>0?10:0}}>
            <input type="number" min="0" value={goalInput} onChange={e=>setGoalInput(e.target.value)} placeholder="Nastav cíl na tento týden" style={{...inputStyle,flex:1,fontFamily:"'DM Mono',monospace"}}/>
        ...
        <div style={{...card,marginBottom:"1rem",padding:"1rem"}}>
          <div style={{display:"flex",alignItems:"flex-end",gap:3,height:64}}>
            {chartDays.map((d,i)=>{
              const sc=chartScores[i],h=sc>0?Math.max(4,Math.round((sc/maxSc)*56)):2;
              return<div key={d} style={{flex:1}}><div title={`${d}: ${sc.toFixed(1)} b`} style={{width:"100%",height:h,background:d===todayStr()?"#38bdf8":sc>0?"#c084fc":"var(--color-border-tertiary)",borderRadius:"3px 3px 0 0"}}/></div>;
            })}
          </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5,marginBottom:"1.25rem"}}>
          {ACTIVITY_META.filter(a=>totals[a.key]>0).map(a=>(
            <div key={a.key} style={{padding:"10px 12px",background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-md)"}}>
              <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:4}}><div style={{width:20,height:20,borderRadius:5,display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,background:a.color+"22",color:a.color,fontWeight:700}}>{a.icon}</div><span style={{fontSize:11,color:"var(--color-text-secondary)"}}>{a.label}</span></div>
              <p style={{margin:0,fontSize:16,fontWeight:700,fontFamily:"'DM Mono',monospace",color:"var(--color-text-primary)"}}>{a.unit==="km"?totals[a.key].toFixed(1):Math.round(totals[a.key])}<span style={{fontSize:11,fontWeight:400,color:"var(--color-text-secondary)",marginLeft:3}}>{a.unit}</span></p>
            </div>
          ))}
        </div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
